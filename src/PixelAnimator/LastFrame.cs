...
public static class LastFrame
{
    public static PngBuilder GetLastFrame(this Png file)
    {
        ...
        for (var x = 0; x < file.Width; x++)
        {
            ...
            builder.SetPixel(pixel > 0 ? white : black, x * 2, y * 2);
            builder.SetPixel(pixel.HasFlag(PixelEdge.Right) ? white : black, x * 2 + 1, y * 2);
            builder.SetPixel(pixel.HasFlag(PixelEdge.Down) ? white : black, x * 2, y * 2 + 1);
            // builder.SetPixel(pixel.HasFlag(PixelEdge.Down | PixelEdge.Right) ? white : black, x * 2 + 1, y * 2 + 1);
            builder.SetPixel(pixel.HasFlag(PixelEdge.Down | PixelEdge.Right | PixelEdge.DownRight) ? white : black, x * 2 + 1, y * 2 + 1);
        }
